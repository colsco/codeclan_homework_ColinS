...
```
```{r}
project <- read_csv(here("data/project_management.csv")) %>% clean_names()
...
project %>% 
  ...
  geom_point() +
  theme_bw() +
  labs(title = "Estimated Project Length v Actual Project Length",
       x = "Estimated Length",
       y = "Actual Length")
```
...
```
The correlation coefficient for estimated length v actual length is 0.80.  From 
this, we can conclude;

* The positive value shows that actual_length increases as estimated_length 
increases
* 0.80 shows a strong correlation between the two variables.
...
```{r}
project_time_model <- lm(formula = actual_length ~ estimated_length, 
                         data = project)
project_time_model
```
The coefficients show that the line of best fit will be;
$$ {actual\ time} = (1.223 * {estimated\ time}) + 1.416 $$
...
```{r}
summary(project_time_model)
```

* The gradient of the line is + 1.2235.  This indicates that for every unit increase 
in actual_time, the estimated_time will increase by 1.2235.

* The Adjusted R-squared value shows that 64% of variance in the output of the 
model can be explained by variance in the predictor.
...
```{r}
autoplot(project_time_model)
```

The diagnostic plots look acceptable.

Residuals vs Fitted
The residuals are generally randomly scattered around a value of 0. 

Normal Q-Q
All the points to lie close to the line, which shows that the residuals are 
generally normally distributed. 

Scale-Location
The residuals are located in a band of fixed width above the x-axis.

The p-value for this model is below 0.05, which in conjunction with the diagnostic
plots above suggests that the model is a statistically significantly good fit for
the project data.
...
project %>% 
  ...
  aes(x = estimated_length, y = actual_length) +
  geom_point() +
  geom_text_repel(aes(label = 1:nrow(project)), size = 3) +
  theme_bw() +
  labs(title = "Estimated Project Length v Actual Project Length",
       x = "Estimated Length",
       y = "Actual Length")
...
Possible outliers in the plot above are 5 and 18.
