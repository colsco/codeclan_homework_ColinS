...
```
Yes, the Thought Police are mentioned.
...
```{r}
pattern_bb <- "(?i)Big Brother"

str_extract_all(whole_text, pattern_bb)
```
There are 20 references to "Big Brother".
...
```{r}
pattern_julia <- "(?i)julia"

str_detect(whole_text, pattern_julia)
```
No, there's no mention of Julia.
...
```{r}
pattern_date <- "[A-Z][a-z+ 0-9]+[a-z]+, [0-9]{4}+"

str_extract(whole_text, pattern_date)
```
...
```{r}
pattern_name_redact <- "(?i)winston( smith)*"  # the normal parenthesis will look for " smith".  In conjunction with "*" it will do nothing if there's nothing found, but will add "smith" to the replacement "REDACTED" when it is found.

str_replace_all(whole_text, pattern_name_redact, replacement = "REDACTED")
```
...
```{r}
 ...
 str_replace_all(whole_text, c("Gin" = "Vodka", "gin" = "vodka", "GIN" = "VODKA"))
...
#  Started out with what's shown below simply to have a working model from which I could develop more robust expressions, but I struggled!
# I think it works correctly and performs the task required, but it is not an elegant solution.
...
str_extract_all(whole_text, "(?i)department*") # there are three occurrences of department/departments

str_extract_all(whole_text, "(?i)ministr+") # there are 12 occurrences of ministry/ministries

str_extract_all(whole_text, "(?i)section*") #there are no occurrences of section/sections

str_replace_all(whole_text, c("Department" = "Section", 
                              "department" = "section", 
...
#str_extract_all(whole_text, "Newspeak")

#str_replace_all(whole_text, c("Minitrue" = "Depitrue",
#                              "Minipax" = "Depipax",
#                              "Miniluv" = "Depiluv",
#                              "Miniplenty" = "Depiplenty"
#                              ))



str_replace(whole_text, "[Mm]ini[tpl]+", replacement = "Depi")   # This loses the "t" so "Minitrue" becomes "Depirue"...?  Others look ok?


```


*I spent quite a lot of time on this, but couldn't come up with an expression that didn't delete some letters that I wanted to keep.  I'll look forward to seeing the solution in the morning!*
