...
Does this improve with a higher threshold?  Try 0.6;
```{r}
threshold <- 0.6
...
Check the residuals;
```{r}
juice_remaining_resid <- juice_train_trim %>%
  add_residuals(juice_1a) %>%
  select(-c("purchase", "loyal_ch"))

juice_remaining_resid %>%
  ggpairs()
```

The plot of residuals in ggpairs implies that `price_ch` might be significant;
...
Does `price_mm` add significance and is it more significant than `price_ch`?
...
```{r}
juice_remaining_resid <- juice_train_trim %>%
  add_residuals(juice_2b) %>%
  select(-c("purchase", "loyal_ch", "price_mm"))
...
The plot of residuals shows `price_ch` is still by far the most significant
remaining factor, but this has already been shown to be insignificant with a
p-value of 0.35.
...
The ROC curve suggests that the 2-predictor model is very mrginally better than
the single predictor model (i.e. it is closer to the top left corner).
...
The AUC also backs up the first impressions of the two-predictor model being a
...
The Gini coefficient also backs the theory that the two predictor model is a 
better fit to the training data.
...
2. Apply the model;
```{r}
juice_model <- glm(purchase ~ loyal_ch + price_mm, data = juice_test_trim,
                family = binomial(link = "logit"))
...
conf_table
```
The model predicted 110 true negatives and 63 true positives, with an accuracy;
...
An overall accuracy of 80% is the sae as the training set, which suggests minimal
overfitting.
...
The True Positive Rate is 83%, which seems quite good.
...
A True Negative Rate of 75% also corresponds well with the training set.


#### Summary

Overall a two-predictor model has been found to be the best model for predicting
whether or not a customer might buy Minute Maid or Citrus Hill, according to the
relationship;

$$purchase_{(MM\ or\ CH)} \sim CH\_brand\_loyalty + price\_MM$$